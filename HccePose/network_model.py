...
class HccePose_BF_Net(nn.Module):
    ...
    def inference_batch(self, inputs, Bbox, thershold=0.5):
        ...
        #移除通道[batch_size, 1, height, width] --> [batch_size, height, width]
        pred_mask = pred_mask[:, 0, ...]

        # 3. 原始编码提取
        #首先将张量从 [batch, 48, H, W] 转换为 [batch, H, W, 48]
        #归一化
        #分离前表面编码和后表面编码
        ...
        return {
            'pred_mask' : pred_mask,                        # 预测掩码
            'coord_2d_image' : coord_image,                 # 2D坐标图像
            'pred_front_code_obj' : pred_front_code_0,      # 前表面3D坐标（实际坐标）
            'pred_back_code_obj' : pred_back_code_0,        # 后表面3D坐标（实际坐标）
            'pred_front_code' : pred_front_code,            # 前表面3D坐标（归一化）
            'pred_back_code' : pred_back_code,              # 后表面3D坐标（归一化）
            'pred_front_code_raw' : pred_front_code_raw,    # 原始前表面编码
            'pred_back_code_raw' : pred_back_code_raw,      # 原始后表面编码
        }

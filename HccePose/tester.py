...
class Tester():
    ...
    def predict(self, cam_K, img, obj_ids, confidence_threshold=0.75, conf=0.75, iou=0.50, max_det=200, pnp_op='ransac+comb'):
        ...
        # STEP 1. 图像预处理
        #       * 根据图像大小进行缩放，确保最大边不超过640像素
        #       * 对图像进行填充使其尺寸为32的倍数，便于网络处理
        #       * 相机内参矩阵也会相应调整以匹配图像变换

        # 1.1 获取图像尺寸
        height, width = img.shape[:2]
        ratio_ = 1
        # 1.2 计算缩放比例，确保最长边不超过640
        ...
        # STEP 2. 物体检测 YOLO
        #       * 使用YOLO模型检测图像中的物体
        #       * 获取检测框坐标(xywh格式)、置信度和类别信息
        #       * 筛选出指定类别的物体(obj_id)

        # 2.1 将图像转换为PyTorch张量并放到设备上
        ...
        with torch.amp.autocast('cuda'):
            ...
            # 2.5 转换坐标格式为(left, top, width, height)
            ...
            for key_i in self.bop_dataset_item.obj_id_list:
                if key_i in obj_ids:
                    ...
                    # STEP 3. 图像裁剪和变换
                    #       * 根据检测到的边界框，计算变换矩阵
                    #       * 将原图中检测到的目标区域裁剪并变换为固定尺寸(默认256x256)的图像块
                    #       * 同时生成较小尺寸(128x128)的图像块用于后续处理  
                    ...
                    if xywh_s.shape[0] > 0:
                        ...
                        # STEP 4. 姿态估计
                        #       * 使用HccePose网络对每个裁剪后的图像块进行推理
                        #       * 预测包括：
                        #         * 掩码(mask)
                        #         * 前表面编码(front code)
                        #         * 后表面编码(back code)
                        #         * 2D坐标图像(coord_2d_image)    
                                            
                        # 4.1 使用HccePose模型进行姿态估计
                        ...
                        # STEP 5. PnP位姿求解
                        #       * 利用预测的掩码、编码和2D坐标信息结合相机参数
                        #       * 使用solve_PnP或solve_PnP_comb函数计算物体的6D姿态(Rotation + Translation)
                        #       * 支持多种PnP算法：EPnP, RANSAC等    
            ...
            #       * 在原图上绘制2D检测框和置信度
            #       * 生成三种可视化结果：
            #         * show_6D_vis0: 显示裁剪区域的姿态估计结果
            #         * show_6D_vis1: 在原图上显示姿态估计结果
            #         * show_6D_vis2: 另一种形式的原图姿态可视化
